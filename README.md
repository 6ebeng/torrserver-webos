...
- The official 32-bit ARM (`arm7`) TorrServer build is **bundled in the `.ipk`** and is the default — webOS userspace is 32-bit ARM on every TV, so a single binary covers all sets and **works offline** with no download on first launch. The exact binary is pinned and SHA256-verified at build time (see `scripts/build.ps1`).
- **Optional downloads:** the **Update server** button and **Select version** picker download a newer/different TorrServer release from GitHub *only when you ask* — the bundled build never requires the network. Before any download the app checks that the data partition has enough free space (about twice the download size) and stops with an out-of-space message rather than writing a truncated binary. A downloaded version is kept across restarts until you pick another; the bundled build is restored on the next app update.
